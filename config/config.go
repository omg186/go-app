...
func loadYaml() {
	...
	if !ok {
		...
		fmt.Errorf("failed to load configuration: Failed to obtain the current file directory")
		return
	}
	...
	if err != nil {
		...
		fmt.Errorf("failed to read configuration file")
		return
	}
	err = yaml.Unmarshal(cfg, &Config)
	if err != nil {
		//panic("Failed to load configuration:" + err.Error())
		fmt.Errorf("failed to load configuration")
		return
	}
}
...
func copyConf(exampleConfig, config string) {
	...
	if err == nil {
		...
		fmt.Errorf("配置文件目录存在同名的文件夹，无法创建配置文件")
